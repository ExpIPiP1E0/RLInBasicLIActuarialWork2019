...
class Environment(object):
    ...
    def step(self, action, auto_reset=True, verbose=False):
        ...
        dreserve_bonus = (1 - self.done) * action * (self.hurdle + 0.01)
        self.reserve_bonus = self.reserve_bonus * (1 + self.hurdle) + dreserve_bonus
        ...
        dreserve_bonus[np.logical_and(0 <= self.cash, self.done)] -= self.reserve_bonus[np.logical_and(0 <= self.cash, self.done)]
        ...
        if auto_reset and np.any(self.done):
            ...
            self.reserve_bonus[self.done] = 0
    ...
    def reset(self):  # 内部的には実質，t,cashの2つのみが状態に相当する．他は全てこれらから導出できる．
        ...
        self.reserve_bonus = np.zeros(shape=self.n_agents)
    ...
    def state(self):  # 外部とやりとりするのは，現在cash, 将来CFのみである．
        ...
        return np.copy(self.cash.reshape(-1, 1)), future_CF, np.copy(self.reserve_bonus.reshape(-1, 1))
