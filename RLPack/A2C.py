...
def gen_model(input_shapes=[[10], [20]], n_outputs=3,
    ...
    policy_head = layers.Dense(n_outputs, activation='softmax', name='policy_head')(x)
    value_head = layers.Dense(1, activation='linear', name='value_head')(x)

    if output_reg:
        value_head = layers.Lambda(lambda x: output_min + (output_max-output_min) * x)(value_head)
...
class Agent(object):
    ...
    def get_action(self, state, greedy=False, get_log=False):
        ...
        ps, value = self.model.predict([*state])  # ps.shape = N * n_actions
        if not greedy:
            if get_log:
                return np.array([[np.random.choice(np.arange(ps.shape[1]), p=p), ] for p in ps]), ps, value
            else:
                return np.array([[np.random.choice(np.arange(ps.shape[1]), p=p), ] for p in ps])
        else:
            if get_log:
                return np.argmax(ps, axis=-1).reshape(-1, 1), ps, value
            else:
                return np.argmax(ps, axis=-1).reshape(-1, 1)
...
class Trainer(object):
    ...
    def train(self, n_steps, n_advantages,
        ...
        for t in tqdm(range(n_steps)):
            ...
            if get_log:
                action, ps, value = self.agent.get_action(state_current, greedy=False, get_log=get_log)
            else:
    ...
    def _fit_on_batch(self, gamma):
        ...
        # model学習
        action = np.concatenate([A[:, 0] for A in self.action], axis=0).reshape(-1, 1)  # n_advantages, n_agents, 1
    ...
    def loss_actor(self, y_true, y_pred):
        ...
        return -self.mult_actor * K.log(K.sum(action_prob*action, axis=1, keepdims=True) + K.epsilon()) \
               * K.stop_gradient(v_target-v_pred)
    ...
    def loss_entropy(self, y_true, y_pred):
        ...
        return self.mult_entropy * K.sum(action_prob * K.log(action_prob + K.epsilon()), axis=-1, keepdims=True)
    ...
    def evaluate(self, show_log=False, time_horizon=1000, n_showing_agents=10, gamma=1.0,
        ...
        for t in range(time_horizon):
            state_current = self.env.state()
            if get_log:
                action, ps, value = self.agent.get_action(state_current, greedy=True, get_log=get_log)
            else:
...
########################################################################################################################
class Tester(object):
    def __init__(self, env, model, name='tester_A2C'):
        self.env = env
        self.model = model
...
class Visualizer(object):
    ...
    This class is specific to this A2C agent, not available for other agents.
    '''
    def __init__(self):
        pass
    ...
    def viz_P_history(self, trainer, data_span_exp=10, data_span_eva=10,
        ...
        sns.heatmap(trainer.log_exp['ps'][::data_span_exp].T, cmap=cmap)
        ...
        plt.figure(figsize=figsize, dpi=dpi)
        sns.heatmap(trainer.log_eva['ps'][::data_span_eva].T, cmap=cmap)
    ...
    def viz_V_history(self, trainer, data_span_exp=10, data_span_eva=10,
                      figsize=(15, 5), dpi=150, cmap='jet', fontsize=16):
        plt.figure(figsize=figsize, dpi=dpi)
        sns.heatmap(trainer.log_exp['value'][::data_span_exp].T, cmap=cmap)
        plt.title('evolution of V in exploration', size=fontsize)
        plt.xlabel('data step', size=fontsize)
        plt.ylabel('action', size=fontsize)
        plt.show()

        plt.figure(figsize=figsize, dpi=dpi)
        sns.heatmap(trainer.log_eva['value'][::data_span_eva].T, cmap=cmap)
        plt.title('evolution of V in evaluation', size=fontsize)
        plt.xlabel('data step', size=fontsize)
        plt.ylabel('action', size=fontsize)
        plt.show()
