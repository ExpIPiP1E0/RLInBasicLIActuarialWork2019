...
class Agent(object):
    '''
    Evolutional Strategyの場合，Agentは必ずしも必要では無いが，API体系を保持するため残している．
    '''
...
class Trainer(object):
    ...
    def train(self, n_episodes, n_samples,
              gamma,
              learning_rate_start=1.0, learning_rate_end=0.1, learning_rate_interval=100,
        ...
        for e in tqdm(range(n_episodes)):
            ...
            learning_rate = self._rate(learning_rate_start, learning_rate_end, learning_rate_interval, e)
            loss = self._fit(n_samples, gamma, epsilon, learning_rate, get_log)
    ...
    def _fit(self, n_samples, gamma, epsilon, learning_rate,
             get_log=False):
        '''
        updated weights in NN by weighted average of noise-add NN by return.
        :param n_samples: number of samples generated for weights of NN
        :param gamma: discount factor
        :param epsilon: size of noise (not sample size)
        :param learning_rate: lr applied to update weight
        :return: total of noise. substantially, no meanings.
        '''
        batch_noise = []
        batch_reward = []
        ...
        for s in range(n_samples):
            ...
            noise_neg = [-noise for noise in noise_pos]
            batch_noise.append(noise_pos)
            batch_noise.append(noise_neg)
        ...
        # normalize batch_reward
        batch_reward = np.array(batch_reward)  # 2 * n_samples
        batch_reward -= np.mean(batch_reward)  # standardize
        std = np.std(batch_reward)
        if 1e-6 < std:
            batch_reward = batch_reward / std

        # calculate weighted average of noise
        weighted_noise = None
        for noise, reward in zip(batch_noise, batch_reward):
            if weighted_noise is None:
                weighted_noise = [reward * n_s for n_s in noise]  # noiseそのものはNNの各重みを格納したリスト
            else:
                for w_n, n_s in zip(weighted_noise, noise):
                    w_n += reward * n_s

        # update model
        for original_weight, noise in zip(original_weights, weighted_noise):
            update = noise / (len(batch_reward) * epsilon)  # see article
            original_weight += learning_rate * update
        self.model.set_weights(original_weights)

        return sum([np.sum(noise) for noise in weighted_noise])
...
########################################################################################################################
class Tester(object):
    def __init__(self, env, model, name='tester_ES'):
        self.env = env
        self.model = model
    ...
    def test(self, n_trials=5, n_episodes=100, n_samples=64,
             gamma=1.0,
             learning_rate_start=0.001, learning_rate_end=0.001, learning_rate_interval=100,
        ...
        for trial in range(n_trials):
            ...
            self.trial(n_episodes, n_samples,
                       gamma,
                       learning_rate_start, learning_rate_end, learning_rate_interval,
    ...
    def trial(self, n_episodes, n_samples,
              gamma,
              learning_rate_start, learning_rate_end, learning_rate_interval,
              epsilon_start, epsilon_end, epsilon_interval,
              verbose, verbose_interval, evaluate_interval,
        ...
        # 学習
        self.trainer.train(n_episodes=n_episodes, n_samples=n_samples,
                           gamma=gamma,
                           learning_rate_start=learning_rate_start, learning_rate_end=learning_rate_end, learning_rate_interval=learning_rate_interval,
